...
fetch('/data/index.json').then(r=>r.json()).then(async idx=>{
  ...
  document.getElementById('article').innerHTML=`<p class="text-sm text-slate-500">${p.date} · ${p.category}</p><h1 class="text-3xl font-bold my-4">${p.title}</h1>${p.contentHtml}<div class="mt-8 text-sm">${p.tags.map(t=>`<span class='px-2 py-1 rounded bg-slate-200 mr-1'>#${t}</span>`).join('')}</div>`;
